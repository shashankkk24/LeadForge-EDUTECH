...
export function ScraperView({ leads, isScraping, scrapeCount, onStartScrape }) {
  ...
  const PLATFORM_INFO = {
    reddit:   { label: 'Reddit (Public JSON API)',    desc: 'Fetches posts from r/edtech, r/education, r/Teachers, r/k12sysadmin and filters by EdTech keywords.' },
    linkedin: { label: 'LinkedIn (Apify)',            desc: 'Scrapes LinkedIn posts mentioning school ERP, LMS, attendance software and other EdTech signals via Apify.' },
    twitter:  { label: 'Twitter / X (Apify)',         desc: 'Scrapes tweets mentioning #edtech, school management, LMS, grading software and K-12 education tools via Apify.' },
  };

  const sorted = [...leads].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
  const recent = filterPlatform === 'all'
    ? sorted
    : sorted.filter(l =>
        filterPlatform === 'linkedin' ? l.platform === 'linkedin' || l.platform === 'linkedin_apify' :
        filterPlatform === 'twitter'  ? l.platform === 'twitter'  || l.platform === 'twitter_apify'  :
        l.platform === filterPlatform
      );
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-sm text-slate-500 mb-8">Scrape Reddit, LinkedIn &amp; Twitter for high-intent EdTech leads in real time.</p>
              ...
              <div className="flex items-end gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    <option value="twitter">Twitter / X (Apify)</option>
                  </select>
                </div>
                <button
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-slate-900" style={{fontFamily:'Manrope,sans-serif'}}>Recently Scraped Leads</h2>
          ...
          {recent.length > 0 && (
            <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
              {recent.length} lead{recent.length !== 1 ? 's' : ''}
            </span>
          )}
        </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wide ${
                        (lead.platform === 'linkedin' || lead.platform === 'linkedin_apify') ? 'bg-blue-700 text-white' :
                        (lead.platform === 'twitter'  || lead.platform === 'twitter_apify')  ? 'bg-sky-500 text-white' :
                                                                                               'bg-orange-100 text-orange-600'
                      }`}>
                        {(lead.platform === 'linkedin' || lead.platform === 'linkedin_apify') ? 'in' :
                         (lead.platform === 'twitter'  || lead.platform === 'twitter_apify')  ? '𝕏'  : 'r/'}
                      </span>
                      <span className="font-semibold text-sm text-slate-900">
                        {lead.platform === 'reddit' ? `u/${lead.username || 'anon'}` : (lead.username || 'anon')}
                      </span>
                    </div>
                  </td>
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-2xl font-black text-slate-900" style={{fontFamily:'Manrope,sans-serif'}}>Lead Intelligence</h1>
          {enriched.length > 0 && (
            <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
              {enriched.length} lead{enriched.length !== 1 ? 's' : ''}
            </span>
          )}
        </div>
              ...
              >
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 capitalize mt-0.5">{lead.detected_role || 'Unknown'} · <span className={`font-bold ${lead.platform === 'linkedin' ? 'text-blue-700' : lead.platform === 'twitter' ? 'text-sky-500' : 'text-orange-500'}`}>{lead.platform}</span></p>
                  </div>
